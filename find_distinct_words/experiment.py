...
def top_x_bottom():
    ...
    for case in cases:
        if case is "tt":
            print("\n\n=========================================================")
            print("                  TOP_j-pop X TOP_k-pop                  ")
            print("=========================================================")

        if case is "tb":
            print("\n\n=========================================================")
            print("                 TOP_j-pop X BOTTOM_k-pop                ")
            print("=========================================================")

        if case is "bt":
            print("\n\n=========================================================")
            print("                 BOTTOM_j-pop X TOP_k-pop                ")
            print("=========================================================")

        if case is "bb":
            print("\n\n=========================================================")
            print("             BOTTOM_j-pop X BOTTOM_k-pop                 ")
            print("=========================================================")
        ...
        for w in range(1, 10):
            ...
            for top_n in range(50, 550, 50):
                # *** Top-N vs. Top-N *** -------------------------------------------------------
                if case is "tt":
                    print("\n****** WEIGHT: {}, TOP-N: {} X 2 (J-pop/K-pop) ******\n".format(weight, top_n))
                    words_ja = ja[:top_n]
                    words_ko = ko[:top_n]
                # --------------------------------------------------------------------------------
                ...
                if case is "bb":
                    print("\n****** WEIGHT: {}, BOTTOM-N: {} X 2 (J-pop/K-pop) ******\n".format(weight, top_n))
                ...
                # *** Top-N vs. Bottom-N *** -------------------------------------------------
                if case is "tb":
                    print("\n****** WEIGHT: {}, TOP-N (J-pop) {} & BOTTOM-N (K-pop) {} ******\n".format(weight, top_n,
                                                                                                        top_n))
                    words_ja = ja[:top_n]
                    words_ko = rev_ko[:top_n]
                # --------------------------------------------------------------------------------
                ...
                # *** Bottom-N vs. Top-N *** -------------------------------------------------
                if case is "bt":
                    print("\n****** WEIGHT: {}, BOTTOM-N (J-pop) {} & TOP-N (K-pop) {} ******\n".format(weight, top_n,
                                                                                                        top_n))
                    words_ja = rev_ja[:top_n]
                    words_ko = ko[:top_n]
                # --------------------------------------------------------------------------------
                ...
                print("Example of selected words:", words[:3])
                tmp_n_w.append(len(set(words)))
                ...
                tmp_ari.append(mean_ari)

            result_ari.append(tmp_ari)
            result_n_w.append(tmp_n_w)

        final_ari = common_func.get_avg(result_ari)
...
def draw_line_graph():
    ...
    with open(in_file_tfidf,"r") as f:
        tfidf = f.read()
    data_tfidf = np.full((10,), np.round(float(tfidf), 5))

    in_file_tt = "table/ari_tt.csv"
    df_tt = pd.read_csv(in_file_tt, header=None)
    df_tt.drop(df_tt.columns[[-1, ]], axis=1, inplace=True)  # Drop last column
    data_tt = np.round(df_tt.iloc[[9]].values[0], 5)

    in_file_bb = "table/ari_bb.csv"
    df_bb = pd.read_csv(in_file_bb, header=None)
    df_bb.drop(df_bb.columns[[-1, ]], axis=1, inplace=True)  # Drop last column
    data_bb = np.round(df_bb.iloc[[9]].values[0], 5)

    in_file_tb = "table/ari_tb.csv"
    df_tb = pd.read_csv(in_file_tb, header=None)
    df_tb.drop(df_tb.columns[[-1, ]], axis=1, inplace=True)  # Drop last column
    data_tb = np.round(df_tb.iloc[[9]].values[0], 5)

    in_file_bt = "table/ari_bt.csv"
    df_bt = pd.read_csv(in_file_bt, header=None)
    df_bt.drop(df_bt.columns[[-1, ]], axis=1, inplace=True)  # Drop last column
    data_bt = np.round(df_bt.iloc[[9]].values[0], 5)

    t = arange(100, 1100, 100)

    plot(t, data_tt, linestyle="-", label="jako_both_top", marker="o")
    plot(t, data_bb, linestyle="--", label="jako_both_bottom", marker="+")
    plot(t, data_tb, linestyle="-.", label="ja_top_ko_bottom", marker="s")
    plot(t, data_bt, linestyle=":", label="ja_bottom_ko_top", marker="x")
